...
public class JavaStreamApi {
    ...
    public Integer oddSum(List<Integer> numbers) {
        ...
        return numbers.stream().filter(o -> o % 2 != 0)
                .reduce((n1, n2) -> n1 + n2)
                .orElse(0);
    }
    ...
    public Long elementCount(List<String> elements, String element) {
        ...
        return elements.stream().filter(o -> o.equals(element)).count();
    }
    ...
    public String findElement(List<String> elements, String element) {
        return elements.stream().filter(e -> e.equals(element))
                .findAny()
    ...
    public Double averageSumOdd(List<Integer> numbers) {
        ...
        return numbers.stream()
                .mapToInt(n -> (numbers.indexOf(n) % 2 != 0 ? n -= 1 : 0))
                .filter(n -> n % 2 != 0)
                .average()
                .orElseThrow(NoSuchElementException::new);
    }
    ...
    public List<People> manSelectByAge(List<People> peopleList, int fromAge, int toAge) {
        return peopleList.stream().filter(p -> (p.getSex().equals(People.Sex.MAN))
     ...
     * Задача: вывести все имена кошек, у которых хозяева это девушки старше femaleAge лет</p>
     **/
    public List<String> getCatsNames(List<People> peopleList, int femaleAge) {
        List<String> names = new ArrayList<>();
        peopleList.stream().filter(people -> people.getSex().equals(People.Sex.WOMEN) && (people.getAge() >= femaleAge))
                .flatMap(p -> p.getCatList().stream().map(c -> names.add(c.getName())))
                .collect(Collectors.toList());
        return names;
    }
}
